...
if ($result->num_rows > 0) {
     echo "<table width'600' border='1' cellspacing='1'><tr><th>Staff_Id</th><th>Staff_name</th><th>Address</th><th>Staff_Salary</th><th>Cell_No</th><th class='dontprint'>Action</th><th class='dontprint'> Action</th><th class='dontprint'>Action</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["Staff_Id"]."</td><td>". $row["Staff_name"]."</td><td>".$row["Address"]."</td><td>".$row["Staff_Salary"]."</td><td>".$row["Cell_No"]."</td><td class='dontprint'><a href='staff_edit.php?Staff_Id=".$row['Staff_Id']."' style='text-decoration: none' />Edit</a></td><td class='dontprint'><a href='staff_delete.php?Staff_Id=".$row['Staff_Id']."' style='text-decoration: none;' />Delete</a></td><td class='dontprint'><a href='staff_print.php?Staff_Id=".$row['Staff_Id']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
