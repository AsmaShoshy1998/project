...
$sql = "SELECT * FROM tb_fees";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
     echo "<table width'600' border='1' cellspacing='1'><tr><th>Fee_id</th><th>Food_Price</th><th>House_Rent</th><th>Total_Expenses</th><th class='dontprint'>Action</th><th class='dontprint'> Action</th><th class='dontprint'>Action</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["Fee_id"]."</td><td>". $row["Food_Price"]."</td><td>".$row["House_Rent"]."</td><td>".$row["Total_Expenses"]."</td><td class='dontprint'><a href='fee_edit.php?Fee_id=".$row['Fee_id']."' style='text-decoration: none' />Edit</a></td><td class='dontprint'><a href='fee_delete.php?Fee_id=".$row['Fee_id']."' style='text-decoration: none;' />Delete</a></td><td class='dontprint'><a href='fee_print.php?Fee_id=".$row['Fee_id']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
