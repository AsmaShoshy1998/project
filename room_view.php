...
if ($result->num_rows > 0) {
     echo "<table width'600' border='1' cellspacing='1'><tr><th>Room_id</th><th>Capacity</th><th>Name_of_Student</th><th class='dontprint'>Action</th><th class='dontprint'> Action</th><th class='dontprint'>Action</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["Room_id"]."</td><td>". $row["Capacity"]."</td><td>".$row["Name_of_Student"]."</td><td class='dontprint'><a href='room_edit.php?Room_id=".$row['Room_id']."' style='text-decoration: none' />Edit</a></td><td class='dontprint'><a href='room_delete.php?Room_id=".$row['Room_id']."' style='text-decoration: none;' />Delete</a></td><td class='dontprint'><a href='room_print.php?Room_id=".$row['Room_id']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
