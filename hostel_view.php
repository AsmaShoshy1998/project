...
$sql = "SELECT * FROM tb_hostel";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
     echo "<table width'600' border='1' cellspacing='1'><tr><th>Hostel_id</th><th>No_of_Room</th><th>No_of_Students</th><th>Location</th><th class='dontprint'>Action</th><th class='dontprint'> Action</th><th class='dontprint'>Action</th></tr>";
     // output data of each row
     while($row = $result->fetch_assoc()) {
         echo "<tr><td>"  . $row["Hostel_id"]."</td><td>". $row["No_of_Room"]."</td><td>".$row["No_of_Students"]."</td><td>".$row["Location"]."</td><td class='dontprint'><a href='hostel_edit.php?Hostel_id=".$row['Hostel_id']."' style='text-decoration: none' />Edit</a></td><td class='dontprint'><a href='hostel_delete.php?Hostel_id=".$row['Hostel_id']."' style='text-decoration: none;' />Delete</a></td><td class='dontprint'><a href='hostel_print.php?Hostel_id=".$row['Hostel_id']."' onclick='myWindow=window.open(this.ref 'newwindow','width=300,height=250')';>Print</a></td></tr>";
     }
     echo "</table>";
} else {
     echo "0 results";
