...
$SQL = "SELECT * FROM tb_hostel  WHERE Hostel_id LIKE '$term'";
  $result = mysqli_query($connection, $SQL);
  if ($result->num_rows > 0) {
     echo "<table width'600' border='1' cellspacing='1'><tr><th>Hostel_id</th><th>No_of_Room</th><th>No_of_Students</th><th>Location</th></tr>";
     // output data of each row
     while($row = mysqli_fetch_array($result)) {
         echo "<tr><td>".$row["Hostel_id"]."</td><td>".$row["No_of_Room"]."</td><td>".$row["No_of_Students"]."</td><td>".$row["Location"]."</td></tr>";
          }
     echo "</table>";
} else {
     echo "0 results";
